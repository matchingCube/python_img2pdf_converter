...
class App(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.statusBar().showMessage(
            "Convert your image files to PDF(Portable Document Format).")
    ...
    def make_pdf_button_on_click(self):
        if self.listWidget.count() == 0:
            ...
            pdf_name, ok = QInputDialog.getText(
                self, "PDF Name", "Give A Name To Your PDF File", QLineEdit.Normal)
            if pdf_name == "":
                QMessageBox.information(
                    self, "Alert", "Please Give Your PDF a name.", QMessageBox.Ok)
                return
            if ok and pdf_name is not None:
                ...
                if reply == QMessageBox.Ok:
                    ...
                    pdf_location = QFileDialog.getExistingDirectory(
                        self, 'Open File')
                    pdf_name += ".pdf"
                    if pdf_location == "":
                        return
                    main.make_pdf_only_selected(
                        items_list, pdf_name, pdf_location)
